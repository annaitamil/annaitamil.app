...
var JSON_PATH = "https://annaitamil.github.io/json/";

var app = phonon.navigator();

app.on({ page: 'home', content: 'home.html' }, function (activity) {
    activity.onReady(function () {
        load(JSON_PATH + "home.json", function (content) {
            var pages = "";
...
app.on({ page: 'thirukkural', content: 'thirukkural.html' }, function (activity) {
    activity.onReady(function () {
        ...
        $("#kural-section").click(function () {
            var items = "";

            json.sections.forEach(function (item, index) {
                items += '<li><a href="#item" class="padded-list title menu">' + item + '</a></li>';
            });

            $('#full-panel-content').html(items);
        });
        ...
        load(JSON_PATH + "thirukkural.json", function (content) {
            json = content;
            update();
        });
    });
...
app.on({ page: 'gnanakural', content: 'gnanakural.html' }, function (activity) {
    activity.onReady(function () {
        ...
        load(JSON_PATH + "gnanakural.json", function (content) {
            json = content;
            update();
        });
    });
...
app.on({ page: 'tamilagaradhi', content: 'tamilagaradhi.html' }, function (activity) {
    activity.onReady(function () {
        ...
        load(JSON_PATH + "tamilagaradhi.json", function (content) {
            json = content;
            update();
        });
    });
...
app.on({ page: 'sivavakkiyam', content: 'sivavakkiyam.html' }, function (activity) {
    activity.onReady(function () {
        ...
        load(JSON_PATH + "sivavakkiyam.json", function (content) {
            json = content;
            index = 0;
            update();
        });
...
app.on({ page: 'vaalai_kummi', content: 'vaalai_kummi.html' }, function (activity) {
    activity.onReady(function () {
        ...
        load(JSON_PATH + "vaalai_kummi.json", function (content) {
            json = content;
            index = 0;
            update();
        });
...
app.on({ page: 'meignana_pulambal', content: 'meignana_pulambal.html' }, function (activity) {
    activity.onReady(function () {
        ...
        load(JSON_PATH + "meignana_pulambal.json", function (content) {
            json = content;
            index = 0;
            update();
        });
...
app.on({ page: 'thirumanthiram', content: 'thirumanthiram.html' }, function (activity) {
    activity.onReady(function () {
        ...
        load(JSON_PATH + "thirumanthiram.json", function (content) {
            json = content;
            index = 0;
            update();
        });
